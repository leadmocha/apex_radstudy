...
RadBox2SD::RadBox2SD(G4String name): G4VSensitiveDetector(name)
{

}

G4bool RadBox2SD::ProcessHits(G4Step *step, G4TouchableHistory *hist)
{
  ...
  //if(step->GetPreStepPoint()->GetStepStatus() == fGeomBoundary || eDep != 0)
    {
      ...
      std::ofstream file;
      file.open(("/scratch/mmk6qa/NewRun4GeV/RadBox2_" + pid_string + ".txt").c_str(), std::ios::app);
      //file.open(("RadBox2_" + pid_string + ".txt").c_str(), std::ios::app);
...
RE01Trajectory* RadBox2SD::GetParentTrajectory(G4int parentID)
{
...
void RadBox2SD::EndOfEvent(G4HCofThisEvent*)
{
