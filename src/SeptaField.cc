...
SeptaField::SeptaField(G4ThreeVector offset, G4RotationMatrix rm) 
	: MappedField( offset, rm, "/home/mmk6qa/SeptumFieldFiles/NewSeptaField_4GeV.txt" ) {
    // Fieldmaps should not be hard coded in!
    SetFilename(gRadConfig->SeptaFieldFilename.data());
	    ReadField();
}


SeptaField::~SeptaField() {
    int i,j,k;
    for( i = 0; i < fN[0]; i++ ){
	for( j = 0; j < fN[1]; j++ ){
	    for( k = 0; k < fN[2]; k++ ){
...
void SeptaField::GetFieldValue(const double Point[3],double *Bfield) const {
    double scale[3];
    int idx, jdx;
    int i, j, k;
    double sx, sy, sz;
...
//	    printf("Out of range\n");
    ...
    // Gauss is already in Geant4 units
    //static const double gauss = 1.e-4*tesla;

    //printf("%f %f %f -> %f %f %f\n", point[0]/cm, point[1]/cm, point[2]/cm, Bfield[0]/gauss, Bfield[1]/gauss, Bfield[2]/gauss );
...
void SeptaField::ReadField(){
    printf("SeptaField - Reading in field\n");
    ...
    */
    ...
    printf("SeptaField - Field complete\n");
