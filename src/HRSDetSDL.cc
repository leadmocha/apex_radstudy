...
HRSDetSDL::HRSDetSDL(G4String name): G4VSensitiveDetector(name)
{
  //fIOID = gIO->AddTree(name);
}

G4bool HRSDetSDL::ProcessHits(G4Step *step, G4TouchableHistory *hist)
{
  ...
  //Energy
  //(*gIO)[fIOID].E = step->GetPreStepPoint()->GetKineticEnergy()/MeV;

  //Energy deposited by this step
  //(*gIO)[fIOID].eDep = step->GetTotalEnergyDeposit()/MeV;
  ...
  //(*gIO)[fIOID].theta = fabs((acos(momdir.z())))/deg;
  //(*gIO)[fIOID].phi = atan2(momdir.y(),momdir.x())/deg;

  //Step length
  //(*gIO)[fIOID].stepLength = step->GetStepLength()/cm;
...
RE01Trajectory* HRSDetSDL::GetParentTrajectory(G4int parentID)
{
...
void HRSDetSDL::EndOfEvent(G4HCofThisEvent*)
{
